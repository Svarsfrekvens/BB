...
export const REGEL_RUBRIK: Record<string, string> = {
  ...
  JOUR_FLOOR: "Sovande jour – grundbemanning",
  JOUR_4W: "Jourtid fyra veckor",
  JOUR_MONTH: "Jourtid kalendermånad",
  WEEK_REST: "Veckovila",
  WEEKEND: "Helgmönster",
  WEEKDAY: "Veckodag",
  PROFILE: "Passprofil",
  ...
  JOUR_SERIES: "Jourpass i följd",
  COMPOSITE_LENGTH: "Sammanvägt pass – längd",
  COMPOSITE_JOUR_WINDOW: "Sammanvägt pass – jour i nattfönster",
  COMP_REST: "Kompensationsvila",
  WITHIN_PASS: "Aktivitet inom pass",
...
export function tolkaMotorSchema(schemaJson: string, upp: Uppslag): MotorSchema | null {
  ...
  const pass: MotorPass[] = ((schema.shifts as any[]) || []).map((s, i) => {
    ...
    return {
      ...
      jour: String(s.type || "") === "jour",
      natt: String(s.type || "") === "night" || String(s.type || "") === "jour" || minuter(start) >= 22 * 60 || minuter(slut) <= minuter(start),
